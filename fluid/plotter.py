...
mesh_data = pd.read_csv(r'/home/kyleperez/javastuff/admpm/fluid/test.csv', header=0)
	...
	if (type == "Strain"):
		...
		for i in range( len(x) ):
			...
			if x[i] < (L - c*t):
				...
				y.append(1 - (A/c) * np.sin( omega*(x[i]-L)/c + omega*t ))
...
def animation(i):
	...
	y = mesh_data['pres'][num_nodes*i: num_nodes*(i+1)]
	...
	ax.set(ylim = [1-0.015, 1+0.015], xlim=[xlowbnd,xhighbnd], xlabel='x (cm)', ylabel='Strain',title='Strain v x')
